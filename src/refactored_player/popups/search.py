...
class SearchPopupMixin:
    ...
    def _display_genre_text(self, row: dict[str, Any]) -> str:
        ...
        for key in ("genre", "genres", "categories", "category", "tags"):
            ...
            if rendered:
                return rendered
        return self._normalize_display_list(self._metadata_text_for_field(row, "GENRE").replace("\n", " ").strip())
    ...
    def _open_search_popup(self) -> None:
        ...
        def _search_field_value(row: dict[str, Any], field: str) -> str:
            token = str(field or "").strip().lower()
            if token == "matches":
        ...
        def _finder_min_widths() -> dict[str, int]:
            mins = {"matches": 120}
            for field, label, _width in _finder_columns():
                if field == "matches":
                    continue
                mins[field] = max(180, int((len(label) + 8) * max(1, self._text_font.measure("0"))))
            return mins
